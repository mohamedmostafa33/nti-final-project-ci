...
const Posts: React.FC<PostsProps> = ({
  ...
  const getPosts = async () => {
    ...
    setLoading(true);
    try {
      // Fetch posts from Django API (returns array directly, no pagination)
      const postsData = await postsAPI.list(communityData?.id);
      
      const posts: Post[] = postsData.map((post: any) => ({
        id: post.id,
        communityId: post.communityId, // Updated field name from backend
        creatorId: post.creatorId?.toString() || "", // Updated field name from backend
        creatorDisplayName: post.creatorDisplayText || "Unknown", // Updated field name from backend
        userDisplayText: post.creatorDisplayText || "Unknown", // Updated field name from backend
        title: post.title,
        body: post.body || "",
        numberOfComments: post.numberOfComments || 0, // Updated field name from backend
        voteStatus: post.voteStatus || 0, // Updated field name from backend
        imageURL: post.imageURL || "", // Updated field name from backend
        communityImageURL: post.communityImageURL || "", // Community image for post header
        createdAt: new Date(post.createdAt).getTime(), // Updated field name from backend
      }));
